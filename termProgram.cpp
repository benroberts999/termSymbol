...
*/
{
  ...
  if (tmp < 0)
    return 0; // XXX this not actually correct... can fix later
  else
    return tmp;
}
...
void instructions() {
  std::cout << "----------------------------------\n";
  std::cout
      << "Enter as: J (as decimal), g-factor, then "
      << " either the leading electron config, or just the number of "
         "electrons.\n"
      << "e.g., for J=3/2, g=1.5, and two-electrons in p,d orbitals, enter:\n"
      << " 1.5 1.5 pd\n"
      << "Or:\n 1.5 1.5 2\n"
      << "Put g=0 for any g-factor (wildcard). Put '0' for any orbitals\n";
  std::cout << "(Nb: you may have to ommit orbitals, since leading config not "
            << "always good enough)\n\n";
}
...
int main() {
  ...
  while (true) {
    ...
    if (count == 0) {
      std::cout << "Perhaps g-factor too tight?"
                   " Currently del(g)/g < "
                << delta_g << "\n\n";
    }
  }
